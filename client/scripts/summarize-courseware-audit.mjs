...
const apiReport = await optionalJson('api-safe-normalization-verification.json');
const dbAccess = await optionalJson('db-access-check.json');
const planFileHash = createHash('sha256').update(await readFile(resolve(base, 'plan-4-5-6.json'), 'utf8')).digest('hex');
const apiCurrent = apiReport?.corpus?.inputHashes?.plan === planFileHash;
...
const sources = new Map(audit.records.map(record => [record.id, record]));
if (proposals.length !== plan.length || proposals.some(proposal => {
  const target = sources.get(proposal.id);
  if (!target) return true;
  const expected = auditChangeHash(proposal, target);
...
const result = { generatedAt: new Date().toISOString(), sourceExportedAt: audit.exportedAt,
  ...
  finalQaComplete: questions.every(q => q.reviewed) && reports.find(r => r.grade === 5)?.finalQaComplete === true
    && apiCurrent && apiReport.status === 'pass' && !plan.some(item => item.validatorConflict),
  productionBlocker: 'Firestore RESOURCE_EXHAUSTED confirmed by read-only transaction on 2026-09-03; no writes performed.',
  dbAccessCheck: dbAccess,
  apiAudit: apiCurrent ? { status:apiReport.status, fixturesPassed:apiReport.fixturePassed, fixturesTotal:apiReport.fixtureTotal,
    checked:apiReport.corpus.total, answerIndexChanges:apiReport.corpus.answerIndexChanges, rejectedByGenerationFilters:apiReport.corpus.rejected,
    exceptions:apiReport.corpus.exceptions.length, retentionIssues:apiReport.corpus.retentionIssues.length } : {status:'stale-or-missing'},
...
const report = [
  ...
  `최종 문항/API QA: ${result.finalQaComplete ? '완료' : '미완료(재검증 필요)'}.`, '',
  ...(apiCurrent ? [`생성 API 전수 대조: ${result.apiAudit.checked}문항, 잘못된 정답 변경 ${result.apiAudit.answerIndexChanges}건, 실행 예외 ${result.apiAudit.exceptions}건, 자료 보존 불일치 ${result.apiAudit.retentionIssues}건. 고정 회귀 ${result.apiAudit.fixturesPassed}/${result.apiAudit.fixturesTotal} 통과.`, ''] : []),
  '## 서비스 반영 상태', '',
  ...
  ...(apiCurrent ? [`생성 API의 차시/도형 필터에 의한 제외는 ${result.apiAudit.rejectedByGenerationFilters}문항입니다. 제외는 문항 오류 판정이 아닙니다. 저장된 문제를 학생이 푸는 경로와 생성 API의 필터는 다르며, 제외 목록은 api-safe-normalization-verification.json을 참고하세요.`, ''] : []),
