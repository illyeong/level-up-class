...
const DEFAULT_SHOP_ITEMS = [
  ...
  },
  {
    name: '프리패스 쿠폰',
    price: 2500,
    description: '선생님과 약속한 1회 프리패스를 사용할 수 있습니다.',
    icon: '🍩',
  },
  {
    name: '간식 교환권',
...
export async function applyClassQuickSetup(selectedClass) {
  ...
  if (classData.quickSetupCompleted === true) {
    return {
  ...
  const existingShopNames = new Set(
    shopSnap.docs.map((d) => String(d.data()?.name || '').trim()).filter(Boolean),
  );
