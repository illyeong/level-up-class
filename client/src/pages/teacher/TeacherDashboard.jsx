...
import iconGold from '../../assets/images/icon-gold.png'; // 🌟 골드 아이콘 불러오기
...
function TeacherDashboard() {
  ...
  const [modalType, setModalType] = useState(''); // 'dia_add', 'dia_sub', 'gold_add', 'gold_sub'
  const [selectedIds, setSelectedIds] = useState([]);
  const [amount, setAmount] = useState('');
  const [reason, setReason] = useState('');
  const [searchQuery, setSearchQuery] = useState(''); // 알잘딱깔센: 모달 내 학생 검색
  ...
  const toggleSelectAll = (filteredStudents) => {
    if (selectedIds.length === filteredStudents.length && filteredStudents.length > 0) {
      setSelectedIds([]); // 전부 선택되어 있으면 해제
    } else {
      setSelectedIds(filteredStudents.map(s => s.id)); // 아니면 모두 선택
    }
  };

  // 🌟 3. 트랜잭션 (지급/차감 및 로그 저장) 실행 - 핵심 로직!
  const submitTransaction = async () => {
    if (selectedIds.length === 0) return alert("학생을 최소 1명 이상 선택해주세요.");
    if (!amount || amount <= 0) return alert("올바른 금액을 입력해주세요.");
    if (!reason.trim()) return alert("지급/차감 사유를 입력해주세요. (로그 저장용)");
    ...
    try {
      ...
      batch.set(logRef, {
        ...
        targetIds: selectedIds // 누구에게 줬는지 ID 배열 저장
      });

      // 한방에 DB로 쏘기!
      await batch.commit();

      alert(`성공적으로 ${currencyLabel} ${actualAmount > 0 ? '지급' : '차감'}이 완료되었습니다!`);
      setIsModalOpen(false);
      fetchStudents(); // 화면 새로고침
  ...
  const fetchLogs = async () => {
    try {
      ...
      console.error("로그 불러오기 에러:", error);
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-8 relative">
      ...
      <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div>
          ...
          <p className="text-slate-500 mt-2 text-sm">
            학생들의 재화를 한눈에 보고, 일괄 지급 및 차감을 진행합니다.
          </p>
        </div>
        
        <div className="flex flex-wrap gap-2">
          {/* 알잘딱깔센: 로그 보기 버튼 추가 */}
          <button onClick={fetchLogs} className="bg-slate-800 hover:bg-slate-900 text-white px-4 py-2 rounded-lg font-bold shadow-sm transition-colors mr-2">
            📋 지급/차감 내역 보기
          </button>
          
          <button onClick={() => openModal('dia_add')} className="bg-indigo-50 border border-indigo-200 text-indigo-700 hover:bg-indigo-600 hover:text-white px-4 py-2 rounded-lg font-bold shadow-sm transition-colors">
            💎 다이아 지급
          </button>
          <button onClick={() => openModal('dia_sub')} className="bg-rose-50 border border-rose-200 text-rose-700 hover:bg-rose-600 hover:text-white px-4 py-2 rounded-lg font-bold shadow-sm transition-colors">
            💎 다이아 차감
          </button>
          <button onClick={() => openModal('gold_add')} className="bg-amber-50 border border-amber-200 text-amber-700 hover:bg-amber-500 hover:text-white px-4 py-2 rounded-lg font-bold shadow-sm transition-colors">
            🪙 골드 지급
          </button>
          <button onClick={() => openModal('gold_sub')} className="bg-orange-50 border border-orange-200 text-orange-700 hover:bg-orange-600 hover:text-white px-4 py-2 rounded-lg font-bold shadow-sm transition-colors">
            🪙 골드 차감
          </button>
        </div>
      </div>
      ...
      {isLoading && !isModalOpen ? (
        <div className="text-center py-20 font-bold text-slate-500">데이터를 불러오는 중입니다...</div>
      ) : (
        /* 🌟 4번 요청: 학생 카드 작게 해서 한 화면에 여러 명 보게 (grid-cols 늘림) */
      ...
      {/* =======================================================
          🌟 2번 요청: 다중 선택 및 사유 입력 팝업창 (트랜잭션 모달) 
          ======================================================= */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-fade-in-up">
            ...
            `}>
              <h2>
                {modalType.includes('dia') ? '💎 다이아' : '🪙 골드'} {modalType.includes('add') ? '일괄 지급' : '일괄 차감'}
              </h2>
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
              ...
              <div className="flex-1 border-b lg:border-b-0 lg:border-r border-slate-200 flex flex-col bg-slate-50">
                <div className="p-4 border-b border-slate-200 bg-white flex justify-between items-center gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor="selectAll" className="font-bold text-slate-700 cursor-pointer">전체 선택</label>
                  </div>
                  {/* 알잘딱깔센 검색창 */}
                  <input 
                    type="text" 
                    placeholder="아이디 검색..." 
                    className="border border-slate-300 rounded-lg px-3 py-1.5 text-sm w-32 focus:outline-none focus:border-indigo-500"
                    value={searchQuery}
                ...
                <div className="flex-1 overflow-y-auto p-4 grid grid-cols-2 md:grid-cols-3 gap-3">
                  {filteredStudents.map(student => (
                    ...
                    >
                      <input 
                        type="checkbox" 
                        className="w-4 h-4 mr-3 rounded text-indigo-600 pointer-events-none" 
                        checked={selectedIds.includes(student.id)} 
                        readOnly
                      />
                      <div className="flex-1">
                        <div className="font-bold text-sm text-slate-800">{student.studentCode}</div>
                        <div className="text-[10px] text-slate-500">
                          다이아: {student.diamonds || 0} / 골드: {student.gold || 0}
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="w-full lg:w-80 p-6 bg-white flex flex-col">
                <div className="mb-6 p-4 bg-slate-100 rounded-xl text-center border border-slate-200">
                  <span className="text-slate-500 text-sm font-medium">선택된 학생</span>
                ...
                <div className="mb-4">
                  <label className="block text-sm font-bold text-slate-700 mb-2">
                    {modalType.includes('add') ? '지급' : '차감'}할 금액
                  </label>
                  <input 
                    type="number" 
                    value={amount}
                    onChange={(e) => setAmount(e.target.value)}
                    className="w-full border-2 border-slate-200 rounded-xl px-4 py-3 font-bold text-xl text-slate-800 focus:outline-none focus:border-indigo-500"
                    placeholder="예: 100"
                    min="1"
                  />
                </div>
                ...
                <div className="mb-6 flex-1">
                  <label className="block text-sm font-bold text-slate-700 mb-2">지급/차감 사유 (필수)</label>
                  <textarea 
                    value={reason}
                    onChange={(e) => setReason(e.target.value)}
                    className="w-full h-24 border-2 border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800 focus:outline-none focus:border-indigo-500 resize-none"
                    placeholder="예: 퀴즈 우승 보상, 지각 벌금 등"
                  ></textarea>
                </div>
                ...
                <button 
                  ...
                  `}
                >
                  {modalType.includes('add') ? '지급 실행하기' : '차감 실행하기'}
                </button>
              </div>
                  ...
                  {logs.length > 0 ? logs.map(log => (
                    <tr key={log.id} className="border-b border-slate-100 hover:bg-slate-50">
                      <td className="p-4 text-slate-500 whitespace-nowrap">
                        {log.timestamp ? new Date(log.timestamp.toDate()).toLocaleString() : '방금 전'}
                      </td>
                      <td className="p-4 font-bold">
                        {log.currency === '다이아' ? '💎 다이아' : '🪙 골드'}
                      </td>
                      <td className={`p-4 font-bold ${log.amount > 0 ? 'text-indigo-600' : 'text-rose-600'}`}>
                    ...
                    <tr><td colSpan="5" className="p-8 text-center text-slate-400">기록이 없습니다.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
