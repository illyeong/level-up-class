...
function TeacherLogin() {
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.log("✅ 선생님 로그인 성공!", user.displayName);
      // TODO: 로그인 성공 시 대시보드 이동 (예: navigate('/dashboard');)
      ...
      <div className="bg-white/95 backdrop-blur-sm p-10 rounded-3xl shadow-2xl w-full max-w-md transform transition-all hover:scale-[1.01]">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 mt-2 text-sm">
            우리 반 상점과 아바타 시스템을 관리하기 위해<br/>선생님의 구글 계정으로 로그인해주세요.
          </p>
        </div>
        ...
        <button 
          ...
          className="group relative flex items-center justify-center w-full bg-white border-2 border-slate-200 rounded-xl px-6 py-4 text-slate-700 font-bold text-lg hover:border-indigo-400 hover:bg-indigo-50 hover:text-indigo-600 transition-all duration-300 shadow-sm hover:shadow-md"
        >
          {/* 구글 G 로고 */}
