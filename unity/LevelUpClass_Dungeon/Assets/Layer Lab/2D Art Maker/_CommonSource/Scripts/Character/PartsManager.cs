...
namespace LayerLab.ArtMaker
{
    ...
    public class PartsManager : MonoBehaviour
    {
        ...
        /// 
       /// <summary>
       /// 초기화면 진입 시 캐릭터를 기본 알몸 상태로 만듭니다. (다이아 0원 시작을 위함)
       /// </summary>
       public void ClearClothes()
       {
           // 필수 파츠(피부, 눈, 입, 눈썹)는 0번(기본 디자인)으로 설정합니다.
           EquipParts(PartsType.Skin, 0);
           EquipParts(PartsType.Eyes, 0);
           EquipParts(PartsType.Mouth, 0);
           EquipParts(PartsType.Brow, 0);

           // 나머지 옷, 신발, 장비, 머리 등은 모두 -1(장착 해제)로 설정하여 싹 벗깁니다.
           EquipParts(PartsType.Top, -1);
           EquipParts(PartsType.Bottom, -1);
           EquipParts(PartsType.Boots, -1);
           EquipParts(PartsType.Hair_Short, -1);
           EquipParts(PartsType.Hair_Hat, -1);
           EquipParts(PartsType.Helmet, -1);
           EquipParts(PartsType.Back, -1);
           EquipParts(PartsType.Eyewear, -1);
           EquipParts(PartsType.Beard, -1);
           EquipParts(PartsType.Gloves, -1);
           EquipParts(PartsType.Gear_Left, -1);
           EquipParts(PartsType.Gear_Right, -1);
       }
        public void Start()
        {
            if (autoInit) Init();
